...
//   F1ファイル出力・F7区分色・F8区分（DivisionTools で元帳と共通）・F11計算はボタン化。
//   摘要／業者の切替はキーワード検索欄の横のスイッチ。F3／F4入換は訂正モード内。
...
export function JournalListPage({ variant, accent }: Props) {
  ...
  const rows = applyCond(
    all.filter((r) => (month == null || r.date.split('/')[0] === month) && dt.visible(r) && (!kw || [r.kari, r.kashi, r.tekiyo, r.gyosha ?? ''].some((x) => x.includes(kw)))),
    cond,
  );
  ...
  return (
    <ReportShell
      ...
      tools={[
        ...
        ...dt.tools,
        { label: '計算', onClick: () => toast.show('再計算しました') },
      ]}
      controls={
        <>
      ...
      <div style={{ overflow: 'auto', maxHeight: 'calc(100vh - 330px)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} onClick={() => setEdit(r)} title="クリックで伝票を訂正" style={{ cursor: 'pointer', background: dt.rowBg(r) }}>
                <td style={TD}><div style={{ fontWeight: 700 }}>{r.seq}</div><div style={{ fontSize: 10.5, color: '#9aa5b1' }}>{dt.chip(r)}{r.service.replace(/^\d+ /, '')}</div></td>
