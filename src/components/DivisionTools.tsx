...
import { flattenDivisions, useSession, type DivisionNode } from '../store/session';
...
export function useDivisionTools(all: Voucher[], accent: string) {
  ...
  const tools = [
    { label: colorOn ? '区分色：ON' : '区分色', onClick: () => setColorOn((c) => !c) },
    { label: divs ? `区分：${divs.length}/${usedCodes.length}` : '区分', onClick: openDialog },
  ];
  ...
  const legend = colorOn ? (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', fontSize: 11.5, color: '#5b6773' }}>
      ...
      <span style={{ color: '#9aa5b1' }}>色は「部門情報の変更」の伝票入力区分ごとのカラー設定です。</span>
    </div>
  ) : null;
  ...
  return { colorOn, divs, visible, rowBg, chip, tools, clearButton, legend, modal, filtered: !!divs };
}
